...
async function procesarColaSync() {
  if (!navigator.onLine || !state.config.sheetsUrl || state.colaOffline.length === 0) return;
  ...
  try {
    ...
    if (json.success) {
      ...
      guardarColaLocal();
      if (state.colaOffline.length > 0) {
        procesarColaSync();
      }
    }
  } catch (e) {
    console.warn("Error procesando cola:", e);
  }
}
...
async function sincronizarConSheets(mostrarMensaje = true) {
  ...
  try {
    // 1. Vaciar cola pendiente primero
